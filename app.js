...
async function openAlbum(album, artistName) {
  ...
  `;
  const downloadBtn = view.querySelector(".download");
  downloadBtn.onclick = () => downloadCover(album.id);
  ...
  tracks.forEach((t,i) => {
    ...
    tr.onclick = () => {
      ...
      bodyBefore.style.setProperty('--accent', '#1db954');
    };
    tracklist.appendChild(tr);
  });

  // Ambient glow color change based on cover
  const img = view.querySelector(".album-hero img");
  img.onload = () => updateAmbientGlow(img);
}
